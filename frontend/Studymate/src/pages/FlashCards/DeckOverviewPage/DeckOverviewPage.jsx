import React, { useState, useEffect } from 'react';
import { useParams } from 'react-router-dom';
import { Sparkles, X } from 'lucide-react';
import toast from 'react-hot-toast';

import { useStudy } from '../../../context/StudyContext';
import flashcardService from '../../../services/flashcardService';

import Button from '../../../components/common/Button/Button';
import Flashcard from '../../../components/flashcards/Flashcard';
import './DeckOverviewPage.scss';

const DeckOverviewPage = () => {
  const { id: documentId } = useParams();
  const { generateDeck } = useStudy();

  // State Management
  const [deck, setDeck] = useState([]);
  const [difficulty, setDifficulty] = useState('medium');
  const [count, setCount] = useState(10);
  
  // Distinct Loading States
  const [deckLoading, setDeckLoading] = useState(false); // Initial page load
  const [generatingLoading, setGeneratingLoading] = useState(false); // AI Generation
  
  // UI States
  const [isGenerating, setIsGenerating] = useState(false);

  const options = {
    count: Number(count),
    difficulty,
  };

  // Fetch deck with a 'silent' option to prevent full-page spinners on refresh
  const fetchDeck = async (silent = false) => {
    if (!silent) setDeckLoading(true);
    try {
      const response = await flashcardService.getFlashcardsForDocument(documentId);
      setDeck(response.data);
    } catch (error) {
      console.error('failed to load deck: ', error);
      toast.error('Failed to load flashcard set');
    } finally {
      if (!silent) setDeckLoading(false);
    }
  };

  useEffect(() => {
    if (documentId) fetchDeck(false);
  }, [documentId]);

  // Excellent optimistic update logic here! Kept exactly as you had it.
  const handleDelete = async (id) => {
    const previous = deck;
    setDeck((prev) => prev.filter((set) => set._id !== id)); 
    try {
      await flashcardService.deleteFlashcardSet(id);
      toast.success('Flashcard set successfully deleted');
    } catch (error) {
      setDeck(previous); // Revert on failure
      toast.error('Failed to delete flashcard set');
    }
  };

  const handleCardGeneration = () => {
    setIsGenerating(false); // Close modal instantly
    setGeneratingLoading(true); // Trigger skeleton loader

    const generationPromise = async () => {
      await generateDeck(documentId, options);
      await fetchDeck(true); // Fetch silently in the background
    };

    // Premium non-blocking toast notification
    toast.promise(generationPromise(), {
      loading: 'Crafting your flashcards with AI... ✨',
      success: 'Flashcards generated successfully! 🎉',
      error: 'Failed to create deck.',
    }).finally(() => {
      setGeneratingLoading(false);
    });
  };

  const EmptyDeckIllustration = () => (
    <svg width="160" height="160" viewBox="0 0 160 160" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="80" cy="80" r="60" fill="#F3E8FF" />
      <rect x="55" y="45" width="60" height="80" rx="8" transform="rotate(15 55 45)" fill="#DDD6FE" />
      <rect x="45" y="35" width="60" height="80" rx="8" transform="rotate(-5 45 35)" fill="#C4B5FD" stroke="#F3E8FF" strokeWidth="2" />
      <rect x="50" y="50" width="60" height="80" rx="8" fill="#8B5CF6" stroke="white" strokeWidth="3" shadow="sm" />
      <rect x="60" y="65" width="40" height="4" rx="2" fill="white" opacity="0.8" />
      <rect x="60" y="75" width="30" height="4" rx="2" fill="white" opacity="0.8" />
      <rect x="60" y="85" width="35" height="4" rx="2" fill="white" opacity="0.8" />
      <path d="M115 35L117 42L124 44L117 46L115 53L113 46L106 44L113 42L115 35Z" fill="#8B5CF6" />
      <path d="M35 105L36.5 110L41.5 111.5L36.5 113L35 118L33.5 113L28.5 111.5L33.5 110L35 105Z" fill="#C4B5FD" />
      <path d="M125 100L126 103L129 104L126 105L125 108L124 105L121 104L124 103L125 100Z" fill="#8B5CF6" />
    </svg>
  );

  function EmptyFlashcard() {
    return (
      <div className="empty-flashcard">
        <div className="empty-flashcard__icon-holder">
          <EmptyDeckIllustration />
        </div>
        <h2>No Flashcards generated</h2>
        <p>Reinforce core concepts through a curated series of active-recall study cards.</p>
        <div className="flashcard__option-holder">
          <Button disabled={deckLoading || generatingLoading} onClick={() => setIsGenerating(true)}>
            {generatingLoading ? 'Generating...' : 'Generate flashcards'}
          </Button>
        </div>
      </div>
    );
  }

  function FlashcardOptionsModal() {
    return (
      <div className="options-modal-overlay">
        <div className="options-modal">
          <div className="options-modal__header">
            <h3>Generation Settings</h3>
            <button className="close-btn" onClick={() => setIsGenerating(false)}>
              <X size={20} />
            </button>
          </div>

          <div className="flashcard-options">
            <div className="flashcard-options__group">
              <label>Select difficulty:</label>
              <select value={difficulty} onChange={(e) => setDifficulty(e.target.value)}>
                <option value="easy">Easy</option>
                <option value="medium">Medium</option>
                <option value="hard">Hard</option>
              </select>
            </div>

            <div className="flashcard-options__group">
              <label>Flashcard quantity:</label>
              <input
                type="number"
                value={count}
                onChange={(e) => setCount(Number(e.target.value))}
                max={15}
                min={1}
              />
            </div>
          </div>

          <div className="options-modal__footer">
            <Button onClick={handleCardGeneration} disabled={generatingLoading}>
              Confirm & Generate
            </Button>
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="deck-overview">
      <div className="deck-overview__header">
        <div className="deck-overview__header--text">
          {deck?.length || 0} flashcard set{deck?.length !== 1 ? 's' : ''} available
        </div>
        <div className="flashcard__option-holder">
          <Button
            className={'deck-overview__header--btn'}
            disabled={deckLoading || generatingLoading}
            onClick={() => setIsGenerating(true)}
          >
            {generatingLoading ? 'Generating...' : 'Generate Flashcards'}{' '}
            <Sparkles size={15} color="gold" fill="#fff" />
          </Button>
        </div>
      </div>

      <div className="deck-overview__grid-holder">
        {deckLoading ? (
          <div className="deck-overview__loading">
            <div className="spinner-loader-circle"></div>
            <p className="spinner-text">Flashcard List Loading...</p>
          </div>
        ) : deck.length === 0 && !generatingLoading ? (
          <div className="empty-flashcard-container">
            <EmptyFlashcard />
          </div>
        ) : (
          <>
            {Array.isArray(deck) &&
              deck.map((card, index) => (
                <Flashcard
                  onDelete={handleDelete}
                  card={card}
                  key={card?._id}
                  index={deck.length - index}
                  documentId={documentId}
                />
              ))}
              
            {/* Dynamic Skeleton Loader while AI generates */}
            {generatingLoading && (
              <div className="skeleton-deck-card">
                <div className="skeleton-pulse skeleton-icon"></div>
                <div className="skeleton-pulse skeleton-title"></div>
                <div className="skeleton-pulse skeleton-subtitle"></div>
                <div className="skeleton-pulse skeleton-button"></div>
              </div>
            )}
          </>
        )}
      </div>

      {isGenerating && <FlashcardOptionsModal />}
    </div>
  );
};

export default DeckOverviewPage;